...
pub fn validate_config(config: &MasterConfig) -> ValidationReport {
    ...
    validate_market_data(&config.market_data, &mut report);
    validate_expiry(&config.expiry, &mut report);
    validate_storage(&config.storage, &mut report);
    validate_oms(&config.oms, &mut report);
    validate_matching_engine(&config.matching_engine, &mut report);
    validate_risk_engine(&config.risk_engine, &config.instrument, &mut report);
